...
$dbconfig = mysqli_connect("localhost","root","","artgallary") or die("An Error occured when connecting to the database");

session_start();
if($_SERVER["REQUEST_METHOD"] == "POST"){
    $email_reg=mysqli_real_escape_string($dbconfig,$_POST['email']);
    $details=mysqli_query($dbconfig,"SELECT fname,email FROM user WHERE email='$email_reg'");
    if (mysqli_num_rows($details)>0) { //if the given email is in database, ie. registered
        ...
        $msg = "Please copy the link and paste in your browser address bar". "\r\n"."http://localhost/artgalary/forgot_password_reset.php?key=".$key."&email=".$email_reg;
        $headers = 'From: Online Art Gallery' . "\r\n";
